...
## 10. APIs Involved
`GET /api/v1/audit-logs` (RBAC, Super-Admin/`audit:view`-gated), `GET /tickets/{id}/audit-logs` and `GET /tickets/audit-logs` (ticketing — scoped-by-default with an opt-in centralized mode gated by `ticket:view_global_audit_log`).
...
## 16. Edge Cases
- The Ticket Audit Log page was deliberately reworked into a **scoped-by-default** view (no permission required, role-accurate slice — Staff narrowed to just their own assigned tickets) plus an **opt-in centralized mode** gated by `ticket:view_global_audit_log` — requiring the permission for the whole page would have turned an Override-only grant into "blocks the page outright" for most roles.
