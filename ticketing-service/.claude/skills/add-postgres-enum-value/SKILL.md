...
**The reverse direction of this same drift also happens, and crashes differently — worth checking for even when you didn't just add anything.** `TicketPriority` was found with a genuine mirror-image bug: Postgres's `ticket_priority_enum` type already had a `CRITICAL` label (confirmed via the `pg_enum` query below — 4 labels: `LOW`/`MEDIUM`/`HIGH`/`CRITICAL`), with 9 real tickets in the dev database already using it and a matching `sla_policies` row already configured for it, but the Python `TicketPriority` enum (`app/enums/ticket_enums.py`) only ever declared the first three. This crashes on **read** (`LookupError: 'CRITICAL' is not among the defined enum values`, thrown by SQLAlchemy's `Enum` type deserializing an existing row), not on write like every other example in this file — the browser sees it as a plain "Network Error"/500 with no CORS headers (see `unified-frontend/CLAUDE.md`'s Known Issues for that pattern), which reads nothing like an enum problem. No migration was needed to fix it (Postgres already had the label) — just adding the missing Python member. **If you're debugging an unexplained crash on any enum-backed column and nothing was recently added to the Python enum, run the `pg_enum` query below *before* assuming the bug is elsewhere** — the Postgres type may already have a label the Python side has never heard of, left over from an earlier migration, a hand-run `ALTER TYPE`, or (as here) a value that was always in the DB but never mirrored into code.
