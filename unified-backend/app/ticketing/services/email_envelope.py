...
def build_agent_signature(current_user: User) -> str:
    """
    NOT on the send path anymore. Per-user signatures are now a real,
    user-editable column (`User.signature_html`), inserted client-side
    into the composer at Compose/Reply/Forward open and included as
    ordinary part of whatever the agent already typed — see the
    Compose/Reply/Forward frontend composers, which are the only place
    a signature is appended today. This function and its HTML
    counterpart below are kept only as (a) the renderer the
    `signature_html`-adding migration used to backfill every existing
    user's column so the cutover was invisible, and (b) the default
    seed for a brand-new user who hasn't set one yet. Nothing on the
    send path calls this anymore — reintroducing a call here would
    double-sign every message whose agent already has a `signature_html`.
    ...
    app/dependencies/auth.py's _build_transient_user). Irrelevant to
    today's callers (a one-time migration backfill, and new-user
    seeding) — neither runs against a cache-hit transient User.
    """
