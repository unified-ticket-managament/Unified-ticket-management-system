...
class ThreadResponse(ORMBase):
    ...
    root first — see InteractionService.get_thread). `ordered_thread`
    is `[parent_interaction, *child_interactions]` — a standalone
    interaction with no parent and no children returns it as the sole
    entry, so the frontend doesn't need a separate "no thread" branch.
    """

    parent_interaction: InteractionResponse
    child_interactions: list[InteractionResponse] = Field(default_factory=list)
    ordered_thread: list[InteractionResponse] = Field(default_factory=list)
    reply_count: int = 0
    latest_interaction: InteractionResponse | None = None
