...
class _FakeInteraction:
    def __init__(
        ...
        *,
        ...
        # A real client email arriving is INBOUND — resolve_reply_
        # addresses (called deep inside add_interaction_reply) branches
        # on this to pick which EmailPayload field is the client's
        # address vs. the shared mailbox's; every root here represents
        # that same "client emailed us" case.
        self.direction = InteractionDirection.INBOUND
...
async def test_pending_reply_denies_unrelated_holder_of_the_permission():
    ...
    this is the over-broad grant this fix closes. `requires_delegated_
    access=True` is what every real Reply/Forward/draft call site now
    passes (see InteractionService.add_interaction_reply et al.)."""
...
async def test_pending_reply_allows_folder_share_recipient():
    """Folder Share is the third delegation source, converging on the
    exact same requires_delegated_access gate as Manual/Rule Forward —
    a non-owner with a genuinely shared folder (folder_shared_bypass)
    can reply even with is_forward_recipient=False."""

    item = _FakeInteraction(uuid4(), client_id=uuid4())
    user_b = _FakeUser(uuid4(), "User B", "Team Lead", permissions=[REPLY_EXTERNAL])
...
async def test_pending_archive_permission_alone_sufficient_by_default():
    """The bare access_control function still admits on permission
    alone when a caller doesn't pass requires_delegated_access — this
    is the backward-compatible default every existing, unmodified
    caller keeps. Real Archive itself no longer relies on this default
    (see the next test) — InteractionService.archive_interaction now
    explicitly opts into requires_delegated_access=True."""
...
async def test_pending_archive_now_requires_delegated_access_too():
    """Architecture-conformance fix: Archive used to admit on
    communication:archive alone with NO resource/thread-access check
    at all — any holder could archive any interaction system-wide.
    Real Archive now passes requires_delegated_access=True (mirrors
    Reply), so an unrelated stranger holding the permission is denied
    unless they own the item or have a genuine forward/folder-share
    relationship to its thread."""
    ...
    # But the same stranger, once genuinely folder-shared or forwarded,
    # is admitted exactly like Reply.
    await access_control.ensure_agent_can_view_pending_interaction(
        item, stranger, client_repository=None,
        permission_backed="communication:archive",
        requires_delegated_access=True,
...
# ---------------------------------------------------------
# Group F — Decision 2: Rule-forward recipients use the SAME
# mechanism as manual-forward recipients (forward_access.py)
# ---------------------------------------------------------


def _rule_forward_row(*, parent_interaction_id, ticket_id, recipient_user_ids):
    """
    A Forward row exactly as RuleEngineService._forward_to_employees
    now creates it (post Decision 1) — same interaction_type=FORWARD +
    payload["recipients"] shape _forward_row above already uses for a
    manual forward, plus the extra rule-specific payload keys
    (rule_id, dispatch_status) real rule-forward rows also carry. The
    access check must not care about any of that extra metadata —
    only interaction_type and payload["recipients"] matter.
    """
    ...
    return _FakeInteraction(
        ...
        payload={
            ...
            "recipients": [
                {"user_id": str(uid), "name": "Recipient", "email": "r@example.com"}
                for uid in recipient_user_ids
            ],
            "forwarded_interaction_id": str(parent_interaction_id),
            "rule_id": str(uuid4()),
...
async def test_rule_forward_grants_the_same_reply_access_as_manual_forward():
    """
    Decision 2's core claim, proven: a Rule-created FORWARD row is
    indistinguishable from a manual one to _is_forwarded_to_user, so a
    Rule-forward recipient can reply exactly like a manual-forward
    recipient already can — with zero changes to the access check
    itself.
    """
    ...
    root = _FakeInteraction(root_id, client_id=client_id)
    user_b = _FakeUser(uuid4(), "User B", "Staff", permissions=[REPLY_EXTERNAL])
    rule_forward = _rule_forward_row(
        parent_interaction_id=root_id, ticket_id=None, recipient_user_ids=[user_b.user_id]
    )

    service = _build_service(
        interactions=[root, rule_forward],
        clients_by_id={client_id: _FakeClient(client_id, other_am_id)},
    )

    response = await service.add_interaction_reply(
        interaction_id=root_id,
        request=InteractionReplyRequest(message="Thanks, I'll take it from here."),
        current_user=user_b,
    )
...
async def test_rule_forward_still_denies_an_unrelated_user():
    """Being named nowhere in the Rule-forward's payload.recipients
    must still deny a stranger, exactly as with a manual forward."""

    root_id = uuid4()
    client_id = uuid4()
    other_am_id = uuid4()
    root = _FakeInteraction(root_id, client_id=client_id)
    named_recipient_id = uuid4()
    stranger = _FakeUser(uuid4(), "Stranger", "Staff", permissions=[REPLY_EXTERNAL])
    rule_forward = _rule_forward_row(
        parent_interaction_id=root_id, ticket_id=None, recipient_user_ids=[named_recipient_id]
    )
...
async def test_rule_forward_with_multiple_recipients_on_one_row():
    """Decision 1's 'one row per action, many recipients' shape: a
    single Rule-forward row naming several recipients must grant each
    of them access independently."""
    ...
    rule_forward = _rule_forward_row(
        ...
        ticket_id=None,
        recipient_user_ids=[user_b.user_id, user_c.user_id],
    )
    ...
    for user in (user_b, user_c):
        response = await service.add_interaction_reply(
            interaction_id=root_id,
            request=InteractionReplyRequest(message="On it."),
            current_user=user,
        )
        assert response.parent_interaction_id == root_id
