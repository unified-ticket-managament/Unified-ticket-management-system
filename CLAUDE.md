...
- Auto-creates on first `BREACHED`/`ESCALATED` crossing (`auto_escalate_if_needed`, no-op if already active); manual trigger is gated by ticket visibility/ownership, not by a permission (`ticket:escalate` is defined in the catalog but deliberately unenforced); auto-advances `TEAM_LEAD → MANAGER → SITE_LEAD` if `ack_due_at` lapses (`evaluate_overdue`, same sweep tick).
